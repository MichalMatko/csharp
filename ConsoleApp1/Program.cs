namespace ConsoleApp1
{
    internal class Program
    {
        ...
        public static void AddItem(List<string> itemList, string itemName, int itemCount)
        {
            ...
            {
                var splitted = foundedItem.Split("||");
                var foundedItemName = splitted[0];
                var foundedItemCount = Int32.Parse(splitted[1]);
                var newItemCount = foundedItemCount + itemCount;

                var indexItem = foundedItem.IndexOf(itemName);
                itemList[indexItem] = $"{itemName}||{newItemCount}";
                Console.WriteLine($"Predmet {foundedItemName.ToUpper()} bol zaktualizovany.");
            }
